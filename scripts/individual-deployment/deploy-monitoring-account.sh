...
if [ -f "$MONITORING_CDK_CONTEXT_FILE" ]; then
    ...
    WORKLOAD_ACCOUNT_LIST_DEFAULT=$(cat $MONITORING_CDK_CONTEXT_FILE | jq -r '."accounts-to-allow"')
...
read -p "Sagemaker workload account list, separated by comma [$WORKLOAD_ACCOUNT_LIST_DEFAULT]: " WORKLOAD_ACCOUNT_LIST
WORKLOAD_ACCOUNT_LIST=${WORKLOAD_ACCOUNT_LIST:-$WORKLOAD_ACCOUNT_LIST_DEFAULT}
...
{
  "accounts-to-allow": "$WORKLOAD_ACCOUNT_LIST",
  "home-region": "$REGION",
